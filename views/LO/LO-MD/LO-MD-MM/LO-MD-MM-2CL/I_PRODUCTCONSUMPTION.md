...
| Field | Data Source |
|---|---|
| key `Product` | `cast (Consumption.matnr as productnumber preserving type)` |
| key `Plant` | `Consumption.werks` |
| key `PostingFiscalYear` | `Consumption.gjahr` |
| key `PeriodType` | `Consumption.perkz` |
| key `NumberOfFollowOnRecords` | `Consumption.zahlr` |
