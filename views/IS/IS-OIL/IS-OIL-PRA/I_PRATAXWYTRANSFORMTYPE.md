---
name: I_PRATAXWYTRANSFORMTYPE
description: Production Revenue Accounting Tax WY Transaction Form Type
app_component: IS-OIL-PRA
software_component: SAPSCORE
release_state: released
system_type: S/4HANA Cloud Public Edition
source_available: false
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_PRATAXWYTRANSFORMTYPE')/$value
semantic_en: Production Revenue Accounting Tax WY Transaction Form Type
tags:
  - IS
  - account
  - bo:material
  - component:IS-OIL-PRA
  - interface-view
  - IS-OIL
  - IS-OIL-PRA
  - product
  - transaction
  - metadata-only
---
# I_PRATAXWYTRANSFORMTYPE

**Production Revenue Accounting Tax WY Transaction Form Type**

| Property | Value |
|---|---|
| App Component | `IS-OIL-PRA` |
| Software Component | `SAPSCORE` |
| Release State | Released |
| System Type | S/4HANA Cloud Public Edition |
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_PRATAXWYTRANSFORMTYPE')/$value) |

## Fields

| Field | Key | Association | Via | Source | Type | Description |
|---|---|---|---|---|---|---|
| `TaxTransacFormType` |  | |  |  | `CHAR(1)` | Tax 2.0 - Original / Amended / Unknown Indicator |
