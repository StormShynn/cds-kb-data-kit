...
| Field | Data Source |
|---|---|
| key `CostAnalysisResource` | `cast( cskv.resrc as fis_co_resource preserving type )` |
| key `ControllingArea` | `cast( cskv.kokrs as fis_kokrs preserving type )` |
| key `Language` | `cskv.spras` |
| key `ValidityEndDate` | `cast( cskv.datbi as fis_datbi preserving type )` |
