---
name: I_BANKACCTCHARACTERISTICTEXT
description: Bank Account Characteristic - Text
app_component: FIN-FSCM-CLM-BAM-2CL
software_component: SAPSCORE
release_state: released
system_type: S/4HANA Cloud Public Edition
source_available: false
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_BANKACCTCHARACTERISTICTEXT')/$value
semantic_en: Bank Account Characteristic - Text
tags:
  - FIN
  - account
  - bo:bank
  - component:FIN-FSCM-CLM-BAM-2CL
  - FIN-FSCM
  - FIN-FSCM-CLM
  - FIN-FSCM-CLM-BAM
  - FIN-FSCM-CLM-BAM-2CL
  - interface-view
  - lob:finance
  - metadata-only
---
# I_BANKACCTCHARACTERISTICTEXT

**Bank Account Characteristic - Text**

| Property | Value |
|---|---|
| App Component | `FIN-FSCM-CLM-BAM-2CL` |
| Software Component | `SAPSCORE` |
| Release State | Released |
| System Type | S/4HANA Cloud Public Edition |
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_BANKACCTCHARACTERISTICTEXT')/$value) |

## Fields

| Field | Type | Description |
|---|---|---|
| `BankAccountCharacteristic` | `CHAR(5)` | Bank Account Characteristic |
| `Language` | `LANG(1)` | Language Key |
| `BankAccountCharacteristicDesc` | `CHAR(60)` | Bank Account Characteristic Name |
