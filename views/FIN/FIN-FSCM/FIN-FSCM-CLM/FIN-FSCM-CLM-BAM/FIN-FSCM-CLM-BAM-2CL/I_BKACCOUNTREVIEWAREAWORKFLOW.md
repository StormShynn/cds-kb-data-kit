---
name: I_BKACCOUNTREVIEWAREAWORKFLOW
description: Bank Account Review Area
app_component: FIN-FSCM-CLM-BAM-2CL
software_component: SAPSCORE
release_state: released
system_type: S/4HANA Cloud Public Edition
source_available: false
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_BKACCOUNTREVIEWAREAWORKFLOW')/$value
semantic_en: Bank Account Review Area
tags:
  - FIN
  - account
  - bo:bank
  - component:FIN-FSCM-CLM-BAM-2CL
  - FIN-FSCM
  - FIN-FSCM-CLM
  - FIN-FSCM-CLM-BAM
  - FIN-FSCM-CLM-BAM-2CL
  - interface-view
  - lob:finance
  - metadata-only
---
# I_BKACCOUNTREVIEWAREAWORKFLOW

**Bank Account Review Area**

| Property | Value |
|---|---|
| App Component | `FIN-FSCM-CLM-BAM-2CL` |
| Software Component | `SAPSCORE` |
| Release State | Released |
| System Type | S/4HANA Cloud Public Edition |
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_BKACCOUNTREVIEWAREAWORKFLOW')/$value) |

## Fields

| Field | Key | Association | Via | Source | Type | Description |
|---|---|---|---|---|---|---|
| `BankAccountReviewAreaUUID` |  | |  |  | `RAW(16)` | Review Area UUID |
| `BankAccountReviewUUID` |  | |  |  | `RAW(16)` | Bank Account Review UUID |
