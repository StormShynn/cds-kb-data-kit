...
| Field | Data Source |
|---|---|
| key `ControllingArea` | `cast ( _Source.kokrs as fincs_controllingarea preserving type )` |
| key `ConsolidationPrftCtrHierarchy` | `cast ( _Source.hryid as fincs_hryid_profitcenter )` |
| key `ValidityEndDate` | `cast ( _Source.hryvalto as fincs_validityenddate preserving type )` |
| key `ProfitCenter` | `cast ( _Source.prctr as fincs_profitcenter preserving type )` |
| key `PartnerProfitCenter` | `cast ( _Source.pprctr as fincs_partnerprofitcenter preserving type )` |
