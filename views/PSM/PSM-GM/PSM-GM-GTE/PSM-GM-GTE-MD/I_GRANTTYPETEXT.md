---
name: I_GRANTTYPETEXT
description: Grant Type - Text
app_component: PSM-GM-GTE-MD
software_component: SAPSCORE
release_state: released
system_type: S/4HANA Cloud Public Edition
source_available: false
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_GRANTTYPETEXT')/$value
semantic_en: Grant Type - Text
tags:
  - PSM
  - component:PSM-GM-GTE-MD
  - interface-view
  - PSM-GM
  - PSM-GM-GTE
  - PSM-GM-GTE-MD
  - metadata-only
---
# I_GRANTTYPETEXT

**Grant Type - Text**

| Property | Value |
|---|---|
| App Component | `PSM-GM-GTE-MD` |
| Software Component | `SAPSCORE` |
| Release State | Released |
| System Type | S/4HANA Cloud Public Edition |
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_GRANTTYPETEXT')/$value) |

## Fields

| Field | Type | Description |
|---|---|---|
| `GrantType` | `CHAR(2)` | Grant Type |
| `Language` | `LANG(1)` | Language Key |
| `GrantTypeDescription` | `CHAR(30)` | Grant Type Description |
