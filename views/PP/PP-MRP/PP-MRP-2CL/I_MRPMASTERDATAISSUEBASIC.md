...
| Field | Data Source |
|---|---|
| key `Material` | `d.Material` |
| key `Plant` | `d.Plant` |
| key `MRPArea` | `d.MRPArea` |
| key `MRPMasterDataIssueSource` | `cast(coalesce( i.source_of_issue, 'H') as pph_source_of_issue preserving type )` |
| key `MRPMasterDataIssueItemCount` | `cast(coalesce( i.counter, 0 ) as pph_counter preserving type )` |
