...
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_WBSELEMENTDATA_2')/$value) |
...
| `StatusProfile` |  | |  |  | `CHAR(8)` | Status Profile |
| `DistributionProfile` |  | |  |  | `CHAR(6)` | Distribution profile |
| `SchedldBasicEarliestEndDate` |  | |  |  | `DATS(8)` | Earliest scheduled finish date (basic) |
| `SchedldBasicLatestStartDate` |  | |  |  | `DATS(8)` | Latest scheduled start date (basic) |
| `SchedldFcstdEarliestEndDate` |  | |  |  | `DATS(8)` | Earliest scheduled finish date (forecast) |
| `LatestSchedldFcstdStartDate` |  | |  |  | `DATS(8)` | Latest scheduled start date (forecast) |
