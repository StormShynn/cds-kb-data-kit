...
| Field | Data Source |
|---|---|
| key `FiscalYear` | `iseg.gjahr` |
| key `PhysicalInventoryDocument` | `cast(iseg.iblnr as pi_item_document preserving type)` |
| key `PhysicalInventoryDocumentItem` | `cast(iseg.zeili as pi_item_number preserving type)` |
| `Material` | `iseg.matnr` |
| `Plant` | `iseg.werks` |
| `StorageLocation` | `iseg.lgort` |
| `Batch` | `iseg.charg` |
| `RealProductBatch` | `cast( case when ( _ProductPlantBasic.ValuationCategory <> '' and _ProductPlantBasic.IsBatchManagementRequired = '' ) then ( '' ) else ( iseg.charg ) end as charg_d )` |
