...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Two columns so a growing saved list scrolls inside its own half instead of
     pushing the view picker further down the page — same split as .builder-grid. */
  .lib-grid { display: grid; grid-template-columns: minmax(0, 1fr) minmax(300px, 1fr); gap: 24px; align-items: start; }
  @media (max-width: 920px) { .lib-grid { grid-template-columns: 1fr; } }
  ...
  .output-tabs { display: flex; gap: 6px; margin-bottom: 10px; }
  .output-tab {
    background: none; border: 1px solid var(--border); color: var(--text-secondary);
    border-radius: 6px; padding: 6px 12px; font-size: 12px; cursor: pointer;
  }
  .output-tab.active { color: var(--text-primary); border-color: var(--accent); background: var(--surface-2); }
  #outputCds {
    width: 100%; min-height: 160px; background: #050505; border: 1px solid var(--border);
    color: #d6f5c3; border-radius: 8px; padding: 16px; font-size: 13px;
    font-family: ui-monospace, "SF Mono", Consolas, monospace; white-space: pre-wrap;
  ...
  .save-panel .save-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; align-items: center; }
  .save-panel .save-actions .copied { color: var(--status-good); font-size: 12px; display: none; }
...
<div class="container">
  ...
  <div class="lib-section collapsed" id="libSection">
    ...
    <div class="lib-grid">
      <div>
        <p class="subhint">Search the shared library plus anything "💾 Save" put in this browser (localStorage). Click a row to load it into the builder.</p>
      ...
      <div class="import-panel">
        <p class="subhint" style="margin:0 0 6px">Already have a query's JSON (from a teammate, a PR, this page's own export)? Paste it here to add it straight to this browser's saved list — no need to rebuild it. Sharing with everyone else is a PR adding the JSON to <code>index/query-library.json</code>.</p>
      ...
      <div class="builder-right">
        <div class="section">
          ...
          <div class="output-tabs">
            <button id="outputTabSql" class="output-tab active" data-role="outputTab" data-tab="sql">SQL / OpenSQL</button>
            <button id="outputTabCds" class="output-tab" data-role="outputTab" data-tab="cds">CDS view (define view entity)</button>
          </div>
          ...
          <pre id="output"></pre>
          <pre id="outputCds" style="display:none"></pre>
          ...
          <div class="save-panel">
            ...
            <p class="subhint" style="margin-top:0">Fill in a title, then <strong>Save</strong> to add it to "📚 Saved queries" above (this browser only). <strong>Share link</strong> copies a URL that carries this whole query, so whoever opens it lands on the builder already filled in. Use <strong>Generate JSON</strong> for the snippet to paste into <code>index/query-library.json</code> via a PR — that does not write to localStorage.</p>
            ...
            <div class="save-actions">
              ...
              <span class="copied" id="shareMsg">Link copied!</span>
            </div>
...
<script>
  ...
  const ABAP_KEYWORDS = new Set(['SELECT', 'FROM', 'WHERE', 'GROUP', 'BY', 'HAVING', 'ORDER', 'JOIN', 'INNER', 'LEFT', 'RIGHT', 'FULL', 'ON', 'AND', 'OR', 'AS', 'IN', 'LIKE', 'NOT', 'IS', 'NULL', 'BETWEEN', 'ASC', 'DESC', 'DEFINE', 'VIEW', 'ENTITY', 'KEY']);
  ...
  // "Generated query" tab 2: same picks as buildQuery(), formatted as a
  // define-view-entity CDS DDL skeleton instead of a plain OpenSQL SELECT.
  // Association rows still need no FROM/JOIN entry in CDS either — same
  // path-expression rule as the SQL tab — so this reuses
  // fromClauseLines()/allFields()/selectState and only changes wording
  // (element list, "key" prefix, "as" aliasing) around them. Called from
  // the tail of buildQuery() so every existing call site keeps both tabs
  // in sync without having to be touched individually.
  function buildCdsView() {
    const out = document.getElementById('outputCds');
    if (!joinedViews.length || !joinedViews[0].name) { out.textContent = ''; return; }

    let elementsText;
    if (isRaw('select')) {
      elementsText = rawText('select') || '*';
    } else {
      const selected = allFields().filter(f => selectState[f.qualifiedName]?.checked);
      const elementLines = selected.map(f => {
        const agg = selectState[f.qualifiedName].agg;
        const expr = agg === AGGS[0] ? f.qualifiedName : agg + '(' + f.qualifiedName + ')';
        const needsAlias = agg !== AGGS[0] || f.qualifiedName !== f.name;
        const keyPrefix = (f.isKey && agg === AGGS[0]) ? 'key ' : '';
        return keyPrefix + expr + (needsAlias ? ' as ' + f.name : '');
      });
      elementsText = elementLines.length ? elementLines.join(',\\n  ') : '*';
    }

    const groupText = isRaw('groupBy') ? rawText('groupBy')
      : allFields().filter(f => groupByState[f.qualifiedName]).map(f => f.qualifiedName).join(', ');
    const whereText = isRaw('where') ? rawText('where') : conditionsText(whereConds.filter(c => c.field));
    const havingText = isRaw('having') ? rawText('having') : conditionsText(havingConds.filter(c => c.field));
    const orderText = isRaw('order') ? rawText('order') : orderConds.map(c => c.field + ' ' + c.dir).join(', ');

    const sqlFrom = fromClauseLines();
    const cdsFrom = ['as select from ' + sqlFrom[0].replace(/^FROM /, '')].concat(sqlFrom.slice(1));

    const lines = ['define view entity Z_MyView', '  ' + cdsFrom.join('\\n  '), '{', '  ' + elementsText, '}'];
    if (whereText) lines.push('where ' + whereText);
    if (groupText) lines.push('group by ' + groupText);
    if (havingText) lines.push('having ' + havingText);
    if (orderText) {
      lines.push('-- "order by ' + orderText + '" omitted: not valid inside a CDS view definition');
      lines.push('-- (views don\\'t guarantee row order) — apply it in the consuming SELECT instead.');
    }

    out.innerHTML = highlightAbap(lines.join('\\n'));
  }

  document.querySelectorAll('[data-role="outputTab"]').forEach(btn => btn.addEventListener('click', (e) => {
    document.querySelectorAll('[data-role="outputTab"]').forEach(b => b.classList.remove('active'));
    e.target.classList.add('active');
    const tab = e.target.dataset.tab;
    document.getElementById('output').style.display = tab === 'sql' ? '' : 'none';
    document.getElementById('outputCds').style.display = tab === 'cds' ? '' : 'none';
  }));

  document.getElementById('copyBtn').addEventListener('click', () => {
    const activeTab = document.querySelector('[data-role="outputTab"].active')?.dataset.tab || 'sql';
    const text = document.getElementById(activeTab === 'cds' ? 'outputCds' : 'output').textContent;
    if (!text) return;
  ...
  // The whole query object is base64url-encoded into #q=… rather than posted
  // anywhere: this page has no backend, and a fragment never leaves the
  // browser (servers don't receive it), so a link is shareable without the
  // query becoming a PR or hitting a third-party service first. Reuses
  // currentQueryAsObject()/loadSavedQuery() so a shared link and a
  // library entry are the exact same shape.
  function encodeShareQuery(obj) {
  ...
  function decodeShareQuery(token) {
  ...
  document.getElementById('shareLinkBtn').addEventListener('click', () => {
    const obj = currentQueryAsObject();
    if (!obj.views.length) {
      showSaveOutput('Pick a view first — there is nothing to share yet.');
      return;
    }
    const url = location.origin + location.pathname + location.search + '#q=' + encodeShareQuery(obj);
    const done = () => {
      const msg = document.getElementById('shareMsg');
      msg.style.display = 'inline';
      setTimeout(() => { msg.style.display = 'none'; }, 1800);
    };
    // location.href is also updated so the address bar itself becomes the
    // shareable link — the fallback for a denied/absent clipboard (file://
    // pages, older browsers) is then just copying it from there, which the
    // textarea below spells out.
    location.hash = 'q=' + encodeShareQuery(obj);
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(done, () => showSaveOutput(url));
    } else {
      showSaveOutput(url);
    }
  });
  ...
  // Opening a "🔗 Share link" URL: decode #q=… into the builder. Runs last,
  // after loadSavedQuery() and the pickers exist. The title/description are
  // also written back into the save panel so the recipient can keep the
  // query with one click on "💾 Save" instead of retyping them.
  (function loadQueryFromHash() {
    const token = /^#q=(.+)$/.exec(location.hash || '');
    if (!token) return;
    let obj;
    try {
      obj = decodeShareQuery(token[1]);
    } catch {
      showSaveOutput('That share link could not be decoded — ask for it again, or paste the query JSON in "📚 Saved queries" above.');
      return;
    }
    if (!obj || !Array.isArray(obj.views) || !obj.views.length) return;
    document.getElementById('saveTitle').value = obj.title || '';
    document.getElementById('saveDesc').value = obj.description || '';
    document.getElementById('saveContributor').value = obj.contributor || '';
    loadSavedQuery(obj);
  })();
</script>
</body>
