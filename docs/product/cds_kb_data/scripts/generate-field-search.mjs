...
<script>
  ...
  function render() {
    ...
    // The query IS a view name, not just a field/table/column it happens to
    // share a name with — checked against every known view (DATA.P), not
    // just ones that show up in F/T/R below. Kept as its own section instead
    // of folded into "As a table/view reference": that section only lists
    // OTHER views that build FROM/associate to the searched name, so typing
    // a view's own name there previously surfaced nothing about the view
    // itself unless some other view happened to reference it — easy to
    // misread as "not found" even though the view exists (this is exactly
    // what came up over I_MaterialStockTimeSeries).
    const isKnownView = key in DATA.P;

    const hasTechnicalMatch = isKnownView || fieldExact || tableExact || rawExact || fieldSubstr.length > 0 || tableSubstr.length > 0 || rawSubstr.length > 0;
    ...
    let html = '';
    if (isKnownView) {
      const ac = (DATA.M[key] || [])[0] || '';
      html += '<div class="section"><h2>This is a CDS view — "' + escapeHtml(key) + '"</h2>' +
        '<div class="row">' + viewLink(key) + unverifiedBadge(key) + unconfirmedBadge(key) + abstractBadge(key) + masterDataBadge(key) +
        (ac ? ' <span class="tag">' + escapeHtml(ac) + '</span>' : '') + '</div></div>';
    }
    if (fieldExact) {
