...
semantic_vi: "EHS Sampling Definition Header Classifier — CDS view tiêu dùng dựa trên I_EHSSamplingHeaderClassifier."
keywords:
  - "ehs"
  - "sampling"
  - "definition"
...
| Source | [View source file](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_EHSSAMPLINGHEADERCLSFRDEX')/$value) |
...
*Source: [https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_EHSSAMPLINGHEADERCLSFRDEX')/$value](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_EHSSAMPLINGHEADERCLSFRDEX')/$value)*

```abap
@AbapCatalog.viewEnhancementCategory: [#NONE]
@AccessControl.authorizationCheck: #CHECK
@EndUserText.label: 'EHS Sampling Definition Header Classifier'
...
@ObjectModel: { 
  ...
  sapObjectNodeType.name: 'EHSSmplgDefHeaderClassifier'
}
...
@Analytics: {
  ...
  dataExtraction: {
    ...
    delta.changeDataCapture: {
      mapping: [{
        table: 'ehenvd_sdef_acla',
        role: #MAIN,
        viewElement: ['EHSSamplingHdrClassifierUUID'],
        tableElement: ['db_key']
      }, {
...
define view entity C_EHSSamplingHeaderClsfrDEX
  as select from           I_EHSSamplingHeaderClassifier as EHSSamplingHeaderClassifier
    left outer to one join I_EHSDataClassifier           as EHSDataClassifier      on EHSSamplingHeaderClassifier.EHSDataClassifierUUID = EHSDataClassifier.EHSDataClassifierUUID
    left outer to one join I_SamplingHeader              as SamplingHeader         on SamplingHeader.SamplingHeaderUUID = EHSSamplingHeaderClassifier.SamplingHeaderUUID
...
{
      @EndUserText.label: 'Sampling Classifier Key'
  key EHSSamplingHeaderClassifier.EHSSamplingHdrClassifierUUID,
...
where
  EHSSamplingHeaderClassifier.EHSDataClassifierIsActive = 'X'
```
