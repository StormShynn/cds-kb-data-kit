...
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_CNSLDTNCASHFLOWQ')/$value
semantic_en: "This query corresponds to the data provider for the Cash Flow Statement tab of the Group Financial Statements Review Booklet app. To help you decide which CDS view to use for your purposes, SAP has introduced the annotation ObjectModel.supportedCapabilities that indicates the most appropriate use cases for each CDS view. To find out what use cases are best supported by this CDS view, access the entry of the CDS view in the View Browser app and find the values for this annotation under the Annotation tab. For more information, see Supported Capabilities for CDS Views."
semantic_vi: "Cash Flow Statement — CDS view tiêu dùng dựa trên Cash Flow Statement."
keywords:
  - "Cash Flow Statement"
  - "cash"
  - "flow"
...
| Source | [View source file](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_CNSLDTNCASHFLOWQ')/$value) |
...
| `PrevFiscalQuarterAmount` |  | |  | `case when ( FiscalYearQuarter = $parameters.P_PreviousFiscalYearQuarter) then 1 end` | `INT1(3)` |  |
...
| `DeltaClsgPrevFsclYTDAmount` |  | |  | `$projection.ClsgFiscalYearAmount - $projection.PrevFiscalYearAmount` | `INT4(10)` |  |
| `DeltaClsgPrevFsclYTDRatio` |  | |  | `ratio_of( portion => $projection.DeltaClsgPrevFiscalYearAmount, total => $projection.PrevFiscalYearAmount) * 100` | `DECF(34)` |  |
...
*Source: [https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_CNSLDTNCASHFLOWQ')/$value](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('C_CNSLDTNCASHFLOWQ')/$value)*

```abap
@AccessControl.authorizationCheck: #NOT_ALLOWED
@Analytics: {
    technicalName: 'CCNSLDTCFSTMT',
...
@EndUserText.label: 'Cash Flow Statement'


define transient view entity C_CnsldtnCashFlowQ
  ...
  with parameters
    ...
    @Consumption.derivation: {
    lookupEntity: 'F_ConsolidationReportingRule',
                   ...
                   ) as MatrixCnsldtnFndnEnhcdCube
{
  ...
  virtual CnsldtnReportingCurrency : abap.cuky,
          ...
          @AnalyticsDetails.query.axis: #COLUMNS
          @UI.fieldGroup: [{ qualifier: 'QuarterComp', groupLabel: 'Quarter Comparison' }]
          case when ( FiscalYearQuarter = $parameters.P_PreviousFiscalYearQuarter) then 1 end                                                                                                                as PrevFiscalQuarterAmount,
          ...
          @AnalyticsDetails.query.decimals:2
          @AnalyticsDetails.query.hidden: true
          ratio_of( portion => $projection.DeltaClsgPrevFiscalYearAmount, total => $projection.PrevFiscalYearAmount) * 100                                                                                   as DeltaClsgPrevFsclYTDRatio,
