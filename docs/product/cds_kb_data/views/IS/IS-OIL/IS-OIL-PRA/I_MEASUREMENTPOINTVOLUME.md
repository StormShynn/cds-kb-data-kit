...
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_MEASUREMENTPOINTVOLUME')/$value) |
...
| `LoadOilTransferWithoutRecovery` |  | |  |  | `CHAR(1)` | Load Oil Transfer without Recovery |
| `MsmtPtVolDocSts` |  | |  |  | `CHAR(1)` | MP Volume Status Code |
| `PRAMeasurementDocumentNumber` |  | |  |  | `CHAR(10)` | Meas. document number |
| `MeasurementDocYear` |  | |  |  | `NUMC(4)` | Meas. document year |
| `DraftMeasurementDocumentNumber` |  | |  |  | `CHAR(10)` | Meas. document number |
| `DraftMeasurementDocYear` |  | |  |  | `NUMC(4)` | Meas. document year |
