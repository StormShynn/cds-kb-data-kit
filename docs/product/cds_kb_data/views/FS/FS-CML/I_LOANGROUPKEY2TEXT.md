---
name: I_LOANGROUPKEY2TEXT
description: "Loan Group Key 2 - Text"
app_component: FS-CML
software_component: SAPSCORE
release_state: released
system_type: S/4HANA Cloud Public Edition
source_available: false
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_LOANGROUPKEY2TEXT')/$value
semantic_en: "Loan Group Key 2 - Text"
tags:
  - FS
  - component:FS-CML
  - FS-CML
  - interface-view
  - metadata-only
---
# I_LOANGROUPKEY2TEXT

**Loan Group Key 2 - Text**

| Property | Value |
|---|---|
| App Component | `FS-CML` |
| Software Component | `SAPSCORE` |
| Release State | Released |
| System Type | S/4HANA Cloud Public Edition |
| Source | [View Hub catalog entry](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_LOANGROUPKEY2TEXT')/$value) |

## Fields

| Field | Key | Association | Via | Source | Type | Description |
|---|---|---|---|---|---|---|
| `Language` |  | |  |  | `LANG(1)` | Language Key |
| `LoanGroupKey2` |  | |  |  | `CHAR(6)` | Group Key 2 (Freely Available) |
| `LoanGroupKey2ShortText` |  | |  |  | `CHAR(50)` | Name |
| `LoanGroupKey2Text` |  | |  |  | `CHAR(15)` | Short Name |
