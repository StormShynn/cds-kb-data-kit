...
source_url: https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_CNCONSETLDETLDSTMNTSUPLRINVC')/$value
semantic_en: "This CDS view is used to see which supplier invoice documents are part of detailed statement information for a corresponding settlement document and/or condition contract. This CDS view provides the data to answer the following business question: Which supplier invoice documents are part of detailed statement information for a corresponding settlement document and/or condition contract? To help you decide which CDS view to use for your purposes, SAP has introduced the annotation ObjectModel.supportedCapabilities that indicates the most appropriate use cases for each CDS view. To find out what use cases are best supported by this CDS view, access the entry of the CDS view in the View Browser app and find the values for this annotation under the Annotation tab. For more information, see Supported Capabilities for CDS Views."
semantic_vi: "Detailed Statement for Supplier Invoice — CDS view tổng hợp dựa trên R_CndnContrSettlmtDetldStmnt."
keywords:
  - "detailed"
  - "statement"
  - "for"
...
| Source | [View source file](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_CNCONSETLDETLDSTMNTSUPLRINVC')/$value) |
...
| `SettledSuplrInvc` | ✓ | |  | `cast( SetldBusVolDocKey1 as wlf_setld_supr_invc )` | `CHAR(10)` | Settled Supplier Invoice |
| `SettledSuplrInvcFiscalYear` | ✓ | |  | `cast( SetldBusVolDocKey2 as wlf_setld_supr_invc_fisc_year )` | `NUMC(4)` | Fiscal Year of a Supplier Invoice |
| `SettledSuplrInvcItem` | ✓ | |  | `cast( SetldBusVolDocItem as wlf_setld_supr_invc_itm )` | `NUMC(6)` | Item of a Settled Supplier Invoice |
...
*Source: [https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_CNCONSETLDETLDSTMNTSUPLRINVC')/$value](https://api.sap.com/odata/1.0/catalog.svc/CdsViewsContent.CdsViews('I_CNCONSETLDETLDSTMNTSUPLRINVC')/$value)*

```abap
@EndUserText.label: 'Detailed Statement for Supplier Invoice'
@AccessControl: {
...
define view entity I_CnConSetlDetldStmntSuplrInvc
  ...
  association [0..1] to I_ConditionContract as _ConditionContract on  $projection.ConditionContract = _ConditionContract.ConditionContract
{
  ...
  key cast( SetldBusVolDocKey1   as wlf_setld_supr_invc )           as SettledSuplrInvc,
  key cast( SetldBusVolDocKey2   as wlf_setld_supr_invc_fisc_year ) as SettledSuplrInvcFiscalYear,
  key cast( SetldBusVolDocItem   as wlf_setld_supr_invc_itm )       as SettledSuplrInvcItem,
