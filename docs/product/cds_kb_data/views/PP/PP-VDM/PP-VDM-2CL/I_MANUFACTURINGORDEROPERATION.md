...
| `MfgOrderOperationOrSubOp` |  | |  | `Operation` |  |  |
| `MfgOrderOperationOrSubOp_2` |  | |  | `Operation_2` |  |  |
...
| `Supplier` |  | |  |  |  |  |
| `PurchaseRequisitionPlndDlvDurn` |  | |  | `PlannedDeliveryDuration` |  |  |
| `GoodsRecipientName` |  | |  | `cast(afvg.GoodsRecipientName as pph_wempf preserving type)` |  |  |
| `UnloadingPointName` |  | |  |  |  |  |
| `MaterialGroup` |  | |  |  |  |  |
| `InspectionLotType` |  | |  |  |  |  |
| `OpExternalProcessingCurrency` |  | |  | `Currency` |  |  |
